...
async fn main() -> anyhow::Result<()> {
    ...
    let users = skynet_users::resolver::UserResolver::new(
        std::sync::Arc::new(std::sync::Mutex::new(rusqlite::Connection::open(db_path)?)),
    );
    let memory = skynet_memory::manager::MemoryManager::new(
        rusqlite::Connection::open(db_path)?,
    );
    let sessions = skynet_sessions::SessionManager::new(
        rusqlite::Connection::open(db_path)?,
    );

    // scheduler: management handle for AppState + engine for background loop
    let scheduler_handle = skynet_scheduler::SchedulerHandle::new(
        rusqlite::Connection::open(db_path)?,
    )?;
    ...
    let state = Arc::new(app::AppState::new(
        config, agent, users, memory, sessions, scheduler_handle, terminal,
    ));
...
fn build_provider(config: &skynet_core::config::SkynetConfig) -> Box<dyn skynet_agent::provider::LlmProvider> {
    ...
    if let Some(ref ollama) = config.providers.ollama {
        ...
        return Box::new(skynet_agent::ollama::OllamaProvider::new(
