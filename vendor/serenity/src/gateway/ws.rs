...
#[cfg(feature = "client")]
use std::io::Read;
// SystemTime removed — since field now uses Option<u64> (unix ms) so Discord
// receives null for non-idle status instead of a broken serde struct.
...
struct PresenceUpdateMessage<'a> {
    afk: bool,
    status: &'a str,
    /// Unix timestamp in milliseconds, or null when not idle.
    /// Discord expects integer|null — not the serde SystemTime struct.
    since: Option<u64>,
